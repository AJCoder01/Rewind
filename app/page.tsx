      ...
      <div className="content">
        ...
        <form className="composer" onSubmit={submit}>
          <div className="field">
            ...
            <textarea id="request" value={request} onChange={(event) => updateRequest(event.target.value)} maxLength={2000} disabled={submitting} />
          </div>
          <button className="primary-button" type="submit" disabled={submitting}>{submitting ? "Preparing…" : "Create World PR"}</button>
        </form>
        {message ? <div className="notice" role="alert">{message}</div> : null}
        <div className="status-row" aria-label="Current slice status">
