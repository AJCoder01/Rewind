...
export async function POST(request: NextRequest, context: { params: Promise<{ worldPrId: string }> }) {
  ...
  if (missingProductionAuthConfiguration().length > 0) return apiError("provider_unavailable", "Dashboard authentication is not configured; no approval was recorded.", requestId, 503, true);
  ...
  try {
    const result = await approveInitialPlan({
    ...
    return apiError("internal_error", "The approval could not be recorded safely; no external action was attempted.", requestId, 500, true);
  }
}
