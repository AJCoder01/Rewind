...
export async function GET(request: NextRequest, context: { params: Promise<{ worldPrId: string }> }) {
  ...
  if (missingProductionAuthConfiguration().length > 0) return apiError("provider_unavailable", "Dashboard authentication is not configured; no World PR can be loaded.", requestId, 503, true);
  ...
  try {
    const view = authorization.actor.source === "mcp"
      ? await getWorldPrStatus(worldPrId, authorization.actor.actorId)
      : await getWorldPr(worldPrId, authorization.actor.actorId);
