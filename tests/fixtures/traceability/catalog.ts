...
const preS058Evidence = [...modelEvidence, "artifacts/test-runs/2026-07-17-pre-s058-correction-pass.md"];
...
const intakeCode = ["app/page.tsx", "app/api/v1/world-prs/route.ts", "lib/services/world-pr.ts", "lib/db/postgres-store.ts", "lib/services/provider-grounded-initial-planner.ts", "mcp/server.ts"];
const intakeTests = ["tests/unit/world-pr.test.ts", "tests/unit/g1-routes-auth.test.ts", "tests/unit/g1-mcp.test.ts", "tests/unit/provider-grounded-initial-planner.test.ts", "scripts/test-e2e.ts"];
...
export const REQUIREMENT_TRACEABILITY: readonly RequirementTrace[] = [
  current({
    ...
    codePaths: intakeCode, testPaths: intakeTests, fixtureIds: initialFixtures, evidencePaths: preS058Evidence,
    status: "partial", note: "The shared service, unsupported-request boundary, and provider-grounded PostgreSQL planner are implemented; the controlled S058 live product run remains the next gate.",
  }),
  current({
    id: "FR-02", kind: "FR", title: "Create idempotency", planTasks: ["S006", "S021"],
    codePaths: ["lib/db/store.ts", "lib/db/memory-store.ts", "lib/db/postgres-store.ts", "lib/services/world-pr.ts", "lib/services/initial-workflow-execution.ts"], testPaths: ["tests/unit/world-pr.test.ts", "tests/unit/g1-memory-store.test.ts", "tests/unit/postgres-store.test.ts", "tests/unit/initial-workflow-execution.test.ts"],
    fixtureIds: ["fixture-initial.v1"], evidencePaths: preS058Evidence, status: "partial",
    note: "Create and initial-execution replay/concurrent-key behavior are covered deterministically without duplicate actions; live replay remains S058.",
  }),
  current({
    id: "FR-03", kind: "FR", title: "Scenario lock and clarification ordering", planTasks: ["S006", "S021", "S077"],
    codePaths: ["lib/db/memory-store.ts", "lib/db/postgres-store.ts", "lib/services/provider-grounded-initial-planner.ts", "lib/domain/fixture-world-pr.ts"], testPaths: ["tests/unit/world-pr.test.ts", "tests/unit/g1-memory-store.test.ts", "tests/unit/postgres-store.test.ts", "tests/unit/provider-grounded-initial-planner.test.ts"],
    fixtureIds: ["acme-demo", "fixture-initial.v1"], evidencePaths: preS058Evidence, status: "partial",
    note: "Provider discovery, rule evaluation, and effect-bearing lock ordering are implemented; later active-rule product tasks remain future work.",
  }),
  current({
    id: "FR-04", kind: "FR", title: "Exactly two controlled Calendar candidates", planTasks: ["S035", "S047"],
    codePaths: ["lib/google/calendar.ts", "lib/domain/calendar-demo.ts", "lib/services/calendar-demo.ts", "lib/services/candidate-resolution.ts", "lib/services/provider-grounded-initial-planner.ts"],
    testPaths: ["tests/unit/google-calendar.test.ts", "tests/unit/calendar-demo.test.ts", "tests/unit/candidate-resolution.test.ts", "tests/unit/provider-grounded-initial-planner.test.ts"],
    fixtureIds: ["traceability.v1"], evidencePaths: preS058Evidence, status: "partial",
    note: "Exact tagged discovery, strict two-candidate validation, deterministic ranking, and the product planner boundary are implemented; the controlled live product use remains S058.",
  }),
  current({
  ...
  current({
    ...
    codePaths: ["lib/domain/fixture-world-pr.ts", "lib/services/candidate-resolution.ts", "lib/services/provider-grounded-initial-planner.ts", "app/pr/[worldPrId]/page.tsx"], testPaths: ["tests/unit/contracts-v1.test.ts", "tests/unit/candidate-resolution.test.ts", "tests/unit/provider-grounded-initial-planner.test.ts", "scripts/test-e2e.ts"],
    fixtureIds: initialFixtures, evidencePaths: preS058Evidence, status: "partial",
    note: "Fixture and provider-grounded ranking/alternative rendering are implemented; S058 remains the live product proof.",
  }),
  current({
    id: "FR-07", kind: "FR", title: "World PR assumption and action preview", planTasks: ["S006", "S050"],
    codePaths: ["lib/domain/fixture-world-pr.ts", "lib/services/initial-reasoning.ts", "lib/services/initial-plan-expansion.ts", "lib/services/provider-grounded-initial-planner.ts", "app/pr/[worldPrId]/page.tsx"], testPaths: ["tests/unit/contracts-v1.test.ts", "tests/unit/initial-reasoning.test.ts", "tests/unit/initial-plan-expansion.test.ts", "tests/unit/provider-grounded-initial-planner.test.ts", "scripts/test-e2e.ts"],
    fixtureIds: initialFixtures, evidencePaths: preS058Evidence, note: "Fixture and provider-grounded previews contain the complete immutable review; S058 supplies the separate live execution proof.",
  }),
  current({
    id: "FR-08", kind: "FR", title: "Exact time, recipient, and content preview", planTasks: ["S006", "S050"],
    codePaths: ["lib/domain/fixture-world-pr.ts", "lib/services/initial-plan-expansion.ts", "lib/services/provider-grounded-initial-planner.ts", "app/pr/[worldPrId]/page.tsx"], testPaths: ["tests/unit/controlled-content.test.ts", "tests/unit/initial-plan-expansion.test.ts", "tests/unit/provider-grounded-initial-planner.test.ts", "scripts/test-e2e.ts"],
    fixtureIds: initialFixtures, evidencePaths: preS058Evidence, note: "Exact time/zone/duration/recipient/mail/brief details are bound and rendered for fixture and provider-grounded plans; S058 supplies the separate live execution proof.",
  }),
  current({
  ...
  current({
    ...
    codePaths: ["lib/contracts/execution-persistence.ts", "lib/services/initial-approval.ts", "lib/services/initial-workflow-execution.ts", "app/api/v1/world-prs/[worldPrId]/approvals/initial/route.ts", "app/api/v1/world-prs/[worldPrId]/execution/route.ts"], testPaths: ["tests/unit/execution-persistence.test.ts", "tests/unit/initial-approval.test.ts", "tests/unit/initial-workflow.test.ts", "tests/unit/initial-workflow-execution.test.ts", "tests/unit/initial-execution-route.test.ts"], fixtureIds: ["initial-plan.v1", "traceability.v1"], evidencePaths: preS058Evidence, status: "partial",
    note: "Exact actor/time/version/digest approval is persisted separately from execution, replayed safely, and consumed only by the dashboard product orchestrator; S058 remains the live proof.",
  }),
  current({
    id: "FR-11", kind: "FR", title: "Approval invalidation on drift", planTasks: ["S051", "S054"],
    codePaths: ["lib/services/initial-execution.ts", "lib/services/initial-calendar-execution.ts", "lib/services/initial-workflow-execution.ts", "lib/services/live-initial-runtime.ts", "lib/contracts/initial-calendar-execution.ts"],
    testPaths: ["tests/unit/initial-execution.test.ts", "tests/unit/initial-calendar-execution.test.ts", "tests/unit/initial-workflow-execution.test.ts"], fixtureIds: ["initial-plan.v1"], evidencePaths: preS058Evidence,
    status: "partial", note: "Whole-plan preflight invalidates pristine approval drift into a fresh unapproved provider-grounded version and stops post-start drift for reconciliation; S058 remains the live proof.",
  }),
  current({
    id: "FR-12", kind: "FR", title: "Durable unique action ledger", planTasks: ["S046", "S052"],
    codePaths: ["lib/contracts/execution-persistence.ts", "lib/db/execution-store.ts", "lib/services/initial-execution.ts", "lib/services/initial-workflow-execution.ts"], testPaths: ["tests/unit/execution-persistence.test.ts", "tests/unit/postgres-execution-store.test.ts", "tests/unit/initial-execution.test.ts", "tests/unit/initial-workflow-execution.test.ts"], fixtureIds: ["initial-plan.v1"], evidencePaths: preS058Evidence,
    status: "partial", note: "Memory/PostgreSQL ledgers enforce immutable plans/approvals/actions, atomic row preparation, claim fences, typed receipts/errors, and terminal replay; S058 remains the live proof.",
  }),
  current({
    ...
    testPaths: ["tests/unit/calendar-primitives.test.ts", "tests/unit/google-calendar.test.ts", "tests/unit/initial-calendar-execution.test.ts", "tests/unit/initial-workflow-execution.test.ts"], fixtureIds: ["traceability.v1", "initial-plan.v1"], evidencePaths: preS058Evidence,
    status: "partial", note: "The approved target/version/static facts and complete whole-plan Calendar/Gmail bindings are rechecked before the first product effect; live product proof remains S058.",
  }),
  current({
    ...
    status: "partial", note: "Start/end-only If-Match writes, sendUpdates=none, owned preparation, changed-ETag verification, receipts, and replay safety are covered deterministically; S058 remains the live product proof.",
  }),
  current({
    ...
    testPaths: ["tests/unit/gmail-delivery.test.ts", "tests/unit/gmail-dispatch-store.test.ts", "tests/unit/gmail-live-proof.test.ts", "tests/unit/gmail-live-proof-store.test.ts", "tests/unit/google-gmail.test.ts", "tests/unit/provider-ports.test.ts", "tests/unit/initial-gmail-execution.test.ts", "tests/unit/initial-workflow-execution.test.ts"],
    fixtureIds: ["traceability.v1", "initial-plan.v1"], evidencePaths: preS058Evidence, status: "partial",
    note: "The product path enforces exact UK—not merely global—allowlisting, dependencies, marker/before-state ownership, typed outcomes, and no-redispatch replay; S058 remains its human live proof.",
  }),
  current({
    id: "FR-16", kind: "FR", title: "Independent brief provenance and exact bytes", planTasks: ["S006", "S014", "S039", "S053"],
    codePaths: ["lib/domain/account-brief.ts", "lib/domain/fixture-world-pr.ts", "lib/contracts/provider-ports.ts", "lib/contracts/v1.ts", "lib/services/account-brief.ts", "lib/services/initial-reasoning.ts", "lib/services/initial-plan-expansion.ts", "lib/services/initial-artifact-execution.ts", "lib/adapters/artifact.ts"], testPaths: ["tests/unit/world-pr.test.ts", "tests/unit/controlled-content.test.ts", "tests/unit/account-brief.test.ts", "tests/unit/provider-ports.test.ts", "tests/unit/initial-reasoning.test.ts", "tests/unit/initial-plan-expansion.test.ts", "tests/unit/initial-artifact-execution.test.ts"],
    fixtureIds: ["controlled-content.v1", "artifact-independence.v1"], evidencePaths: preS058Evidence, status: "partial",
    note: "Planning rejects divergent model-proposed brief bytes, binds canonical source/content hashes, and execution persists exactly the approved bytes without regeneration; S058 remains the live proof.",
  }),
  current({
    ...
    testPaths: ["tests/unit/execution-timeline.test.ts", "tests/unit/initial-workflow.test.ts", "tests/unit/initial-workflow-execution.test.ts", "tests/unit/accessibility-contract.test.ts", "scripts/test-e2e.ts"], fixtureIds: ["initial-plan.v1", "traceability.v1"],
    evidencePaths: preS058Evidence, status: "partial",
    note: "Timeline completion now requires both three succeeded receipts and a completed durable task; product orchestration persists honest completed/attention states. Live proof remains S058.",
  }),
  current({
    id: "FR-18", kind: "FR", title: "Safe retry and resume", planTasks: ["S021", "S052", "S053", "S054", "S055", "S056", "S057"],
    codePaths: ["lib/services/initial-execution.ts", "lib/services/initial-artifact-execution.ts", "lib/services/initial-calendar-execution.ts", "lib/services/initial-gmail-execution.ts", "lib/services/initial-workflow-execution.ts", "lib/services/execution-timeline.ts", "app/components/execution-timeline.tsx"],
    testPaths: ["tests/unit/initial-execution.test.ts", "tests/unit/initial-artifact-execution.test.ts", "tests/unit/initial-calendar-execution.test.ts", "tests/unit/initial-gmail-execution.test.ts", "tests/unit/initial-workflow-execution.test.ts", "tests/unit/execution-timeline.test.ts", "tests/unit/initial-workflow.test.ts"], fixtureIds: ["initial-plan.v1", "traceability.v1"],
    evidencePaths: preS058Evidence, status: "partial",
    note: "Fenced retries, process-death stops, synchronous resume, completed replay, and competing-key serialization are covered deterministically; live proof remains S058.",
  }),
  ...
  current({
    ...
    codePaths: ["app/api/v1/world-prs/[worldPrId]/approvals/initial/route.ts", "app/api/v1/world-prs/[worldPrId]/execution/route.ts", "lib/services/initial-approval.ts", "lib/services/initial-execution.ts", "lib/services/initial-artifact-execution.ts", "lib/services/initial-calendar-execution.ts", "lib/services/initial-gmail-execution.ts", "lib/services/initial-workflow-execution.ts"], testPaths: ["tests/unit/initial-approval.test.ts", "tests/unit/initial-execution.test.ts", "tests/unit/initial-artifact-execution.test.ts", "tests/unit/initial-calendar-execution.test.ts", "tests/unit/initial-gmail-execution.test.ts", "tests/unit/initial-workflow.test.ts", "tests/unit/initial-workflow-execution.test.ts", "tests/unit/initial-execution-route.test.ts"], fixtureIds: ["initial-plan.v1", "traceability.v1"], evidencePaths: preS058Evidence, status: "partial",
    note: "Approval and execution are separate dashboard-only mutations; whole-plan preflight and every action executor require the exact immutable approval. S058 remains the controlled live proof.",
  }),
  ...
  current({
    ...
    codePaths: ["mcp/server.ts", "app/api/v1/world-prs/route.ts", "app/api/v1/world-prs/[worldPrId]/status/route.ts", "app/api/v1/world-prs/[worldPrId]/approvals/initial/route.ts", "app/api/v1/world-prs/[worldPrId]/execution/route.ts"], testPaths: ["tests/unit/g1-mcp.test.ts", "tests/unit/g1-routes-auth.test.ts", "tests/unit/initial-approval.test.ts", "tests/unit/initial-execution-route.test.ts", "scripts/test-e2e.ts"], fixtureIds: ["fixture-initial.v1"], evidencePaths: preS058Evidence, status: "partial",
    note: "MCP exposes only create/safe status and is explicitly rejected by approval, refresh, and execution routes; later recovery/rule/reset endpoints remain future work.",
  }),
  current({
    id: "SAFE-04", kind: "SAFE", title: "Authenticated dashboard and MCP boundaries", planTasks: ["S006", "S022"],
    codePaths: ["lib/auth/session.ts", "app/api/v1/auth/session/route.ts", "app/api/v1/world-prs/route.ts", "app/api/v1/world-prs/[worldPrId]/route.ts", "app/api/v1/world-prs/[worldPrId]/approvals/initial/route.ts", "app/api/v1/world-prs/[worldPrId]/execution/route.ts"], testPaths: ["tests/unit/auth.test.ts", "tests/unit/g1-routes-auth.test.ts", "tests/unit/initial-approval.test.ts", "tests/unit/initial-execution-route.test.ts", "scripts/test-e2e.ts"], fixtureIds: ["fixture-initial.v1"], evidencePaths: preS058Evidence,
    note: "Dashboard session, scope, same-origin, CSRF, idempotency, and MCP separation are covered through initial execution.",
  }),
  current({
    id: "SAFE-05", kind: "SAFE", title: "Controlled account/calendar/recipient boundary", planTasks: ["S010", "S032", "S033", "S034", "S035", "S037"],
    codePaths: ["lib/config/environment.ts", "lib/contracts/oauth.ts", "lib/contracts/provider-ports.ts", "lib/google/oauth.ts", "lib/google/oidc.ts", "lib/google/credentials.ts", "lib/db/oauth-store.ts", "lib/adapters/calendar.ts", "lib/adapters/gmail.ts", "lib/google/calendar.ts", "lib/domain/calendar-demo.ts", "lib/services/calendar-demo-command.ts", "lib/services/live-initial-runtime.ts", "lib/services/initial-workflow-execution.ts"],
    testPaths: ["tests/unit/environment-config.test.ts", "tests/unit/google-identity.test.ts", "tests/unit/oauth-routes.test.ts", "tests/unit/oauth-transaction.test.ts", "tests/unit/oauth-store.test.ts", "tests/unit/provider-ports.test.ts", "tests/unit/google-calendar.test.ts", "tests/unit/calendar-demo.test.ts", "tests/unit/calendar-demo-command.test.ts", "tests/unit/initial-workflow-execution.test.ts"],
    fixtureIds: ["traceability.v1"], evidencePaths: preS058Evidence, status: "partial",
    note: "Exact connected identity/scopes, controlled Calendar, and region-specific recipient bindings are rechecked by the product runtime; S058 remains the live product proof.",
  }),
  current({
    id: "SAFE-06", kind: "SAFE", title: "Calendar ETag conflict protection", planTasks: ["S036", "S054", "S057", "S067"],
    codePaths: ["lib/adapters/calendar.ts", "lib/google/calendar.ts", "lib/services/calendar-primitives.ts", "lib/services/initial-calendar-execution.ts", "lib/services/initial-workflow-execution.ts"], testPaths: ["tests/unit/calendar-primitives.test.ts", "tests/unit/google-calendar.test.ts", "tests/unit/initial-calendar-execution.test.ts", "tests/unit/initial-workflow.test.ts", "tests/unit/initial-workflow-execution.test.ts"], fixtureIds: ["traceability.v1"], evidencePaths: preS058Evidence,
    status: "partial", note: "Initial whole-plan and immediate pre-write checks fail closed on stale state without rebasing; recovery ETag protection remains S067.",
  }),
  current({
    ...
    testPaths: ["tests/unit/gmail-delivery.test.ts", "tests/unit/gmail-dispatch-store.test.ts", "tests/unit/gmail-live-proof.test.ts", "tests/unit/gmail-live-proof-store.test.ts", "tests/unit/google-gmail.test.ts", "tests/unit/provider-ports.test.ts", "tests/unit/initial-gmail-execution.test.ts", "tests/unit/initial-workflow.test.ts", "tests/unit/initial-workflow-execution.test.ts"],
    fixtureIds: ["traceability.v1", "initial-plan.v1"], evidencePaths: preS058Evidence, status: "partial",
    note: "Post-marker ambiguity and lost preparation stop terminally, completed/concurrent replay does not redispatch, and only pre-handoff local failure is retryable; S058/S069 remain live/recovery gates.",
  }),
  current({
    id: "SAFE-08", kind: "SAFE", title: "Closed strict model/action boundary", planTasks: ["S004", "S006", "S034", "S040", "S041", "S042", "S043"],
    codePaths: ["lib/contracts/v1.ts", "lib/contracts/initial-plan-server.ts", "lib/contracts/provider-ports.ts", "lib/contracts/provider-spike.ts", "lib/domain/fixture-world-pr.ts", "lib/ai/model.ts", "lib/ai/model-trusted-facts.ts", "lib/ai/openai-responses.ts", "lib/ai/openai-model.ts", "lib/ai/ollama-chat.ts", "lib/ai/ollama-model.ts", "lib/ai/product-model.ts", "lib/ai/model-schemas.ts", "lib/ai/model-safety.ts", "lib/services/provider-spike.ts", "lib/services/provider-grounded-initial-planner.ts", "lib/services/initial-reasoning.ts", "lib/services/initial-plan-expansion.ts", "scripts/prove-local-model.ts", "scripts/verify-zero-credit-runtime.ts"], testPaths: ["tests/unit/contracts-v1.test.ts", "tests/unit/g1-contracts.test.ts", "tests/unit/provider-ports.test.ts", "tests/unit/world-pr.test.ts", "tests/unit/openai-responses.test.ts", "tests/unit/openai-model.test.ts", "tests/unit/ollama-chat.test.ts", "tests/unit/ollama-model.test.ts", "tests/unit/product-model.test.ts", "tests/unit/model-schemas.test.ts", "tests/unit/model-safety.test.ts", "tests/unit/provider-spike.test.ts", "tests/unit/provider-grounded-initial-planner.test.ts", "tests/unit/initial-reasoning.test.ts", "tests/unit/initial-plan-expansion.test.ts"], fixtureIds: [...initialFixtures, "model-safety.v1"], evidencePaths: preS058Evidence, status: "partial",
    note: "The strict selected runtime now drives provider-grounded product planning, including exact canonical artifact output and no fixture fallback; recovery/rule product integration remains future work.",
  }),
  current({
    id: "SAFE-09", kind: "SAFE", title: "Server-only private environment boundary", planTasks: ["S003", "S012", "S013", "S031", "S032", "S033"],
    codePaths: ["lib/config/environment.ts", "lib/db/config.ts", "lib/ai/product-model.ts", "lib/google/oauth.ts", "lib/google/oidc.ts", "lib/google/credentials.ts", "lib/db/oauth-store.ts", "lib/services/connection-preflight.ts", "lib/services/live-initial-runtime.ts", "db/migrations/0002_oauth_transaction.sql", "scripts/config-check.ts", "scripts/prove-local-model.ts", "scripts/verify-zero-credit-runtime.ts", "scripts/security-scan.ts"], testPaths: ["tests/unit/environment-config.test.ts", "tests/unit/db-config.test.ts", "tests/unit/config-check.test.ts", "tests/unit/product-model.test.ts", "tests/unit/google-identity.test.ts", "tests/unit/oauth-transaction.test.ts", "tests/unit/oauth-migration.test.ts", "tests/unit/oauth-routes.test.ts", "tests/unit/oauth-store.test.ts", "tests/unit/connection-preflight.test.ts", "tests/unit/security-scan.test.ts"], fixtureIds: ["traceability.v1"], evidencePaths: preS058Evidence, status: "partial",
    note: "Server-only configuration/identity/token boundaries and zero-credit local runtime validation are covered; final release log/client leakage remains S089.",
  }),
  current({
    ...
    note: "Synthetic boundaries, encrypted secret handling, safe environment/provider errors, redacted timeline/receipt projections, and credential-safe S058 instructions are covered; complete release logging audit remains S089.",
  }),
  ...
  current({
    ...
    codePaths: ["lib/db/store.ts", "lib/db/memory-store.ts", "lib/db/postgres-store.ts", "lib/db/execution-store.ts", "lib/db/gmail-dispatch.ts", "lib/db/gmail-live-proof.ts", "lib/contracts/execution-persistence.ts", "lib/contracts/initial-artifact-execution.ts", "lib/contracts/initial-calendar-execution.ts", "lib/contracts/initial-gmail-execution.ts", "lib/services/gmail-delivery.ts", "lib/services/gmail-live-proof.ts", "lib/services/initial-execution.ts", "lib/services/initial-artifact-execution.ts", "lib/services/initial-calendar-execution.ts", "lib/services/initial-gmail-execution.ts", "lib/services/initial-workflow-execution.ts", "lib/contracts/execution-shared.ts", "lib/contracts/execution-timeline.ts", "lib/services/execution-timeline.ts", "app/components/execution-timeline.tsx", "app/api/v1/world-prs/[worldPrId]/execution/route.ts"], testPaths: ["tests/unit/world-pr.test.ts", "tests/unit/g1-memory-store.test.ts", "tests/unit/postgres-store.test.ts", "tests/unit/gmail-delivery.test.ts", "tests/unit/gmail-dispatch-store.test.ts", "tests/unit/gmail-live-proof.test.ts", "tests/unit/gmail-live-proof-store.test.ts", "tests/unit/execution-persistence.test.ts", "tests/unit/initial-execution.test.ts", "tests/unit/initial-artifact-execution.test.ts", "tests/unit/initial-calendar-execution.test.ts", "tests/unit/initial-gmail-execution.test.ts", "tests/unit/initial-workflow-execution.test.ts", "tests/unit/execution-timeline.test.ts"], fixtureIds: ["fixture-initial.v1", "initial-plan.v1", "traceability.v1"], evidencePaths: preS058Evidence, status: "partial",
    note: "Completed replay, identical-key replay, different-key concurrency, action fences, and Gmail terminal replay produce one deterministic effect per action; live replay remains S058 and recovery replay remains future work.",
  }),
  current({
    id: "NFR-03", kind: "NFR", title: "Stale Calendar changes never overwrite", planTasks: ["S036", "S054", "S057", "S067"],
    codePaths: ["lib/services/calendar-primitives.ts", "lib/services/initial-calendar-execution.ts", "lib/services/initial-workflow-execution.ts", "lib/adapters/calendar.ts", "lib/google/calendar.ts"], testPaths: ["tests/unit/calendar-primitives.test.ts", "tests/unit/google-calendar.test.ts", "tests/unit/initial-calendar-execution.test.ts", "tests/unit/initial-workflow.test.ts", "tests/unit/initial-workflow-execution.test.ts"], fixtureIds: ["traceability.v1"], evidencePaths: preS058Evidence,
    status: "partial", note: "Stale whole-plan preflight and conditional writes produce zero overwrite and require fresh approval/reconciliation; recovery remains S067.",
  }),
  current({
    ...
    note: "Closed planning, whole-plan preflight, exact regional allowlisting, template, artifact, Calendar, Gmail, and strict model boundaries reject unknown/substituted input before adapters; recovery integration remains planned.",
  }),
  current({
  ...
  current({
    ...
    codePaths: ["lib/domain/digest.ts", "lib/db/migrate.ts", "lib/db/postgres-store.ts", "lib/db/store.ts", "lib/contracts/execution-persistence.ts", "lib/contracts/execution-shared.ts", "lib/contracts/execution-timeline.ts", "lib/services/initial-workflow-execution.ts", "lib/services/execution-timeline.ts", "app/components/execution-timeline.tsx", "app/api/v1/world-prs/[worldPrId]/execution/route.ts"], testPaths: ["tests/unit/contracts-v1.test.ts", "tests/unit/migration-contract.test.ts", "tests/unit/postgres-store.test.ts", "tests/unit/g1-memory-store.test.ts", "tests/unit/execution-persistence.test.ts", "tests/unit/initial-workflow-execution.test.ts", "tests/unit/execution-timeline.test.ts", "tests/unit/initial-workflow.test.ts"], fixtureIds: ["initial-plan.v1", "traceability.v1"], evidencePaths: preS058Evidence, status: "partial",
    note: "Plan/actor/idempotency/action/typed-receipt traceability now spans the product orchestrator and honest task state; recovery evidence is future work.",
  }),
  current({
