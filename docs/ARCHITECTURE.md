...
S046 implements those later persistence semantics in `lib/contracts/execution-persistence.ts` and `lib/db/execution-store.ts` without replacing the foundation tables. Plans and approvals are insert-only application records bound to the canonical payload digest; action rows are created idempotently under the existing `(plan_id, action_key)` constraint; claims use a short lease and increment attempts; terminal receipts/errors clear the lease; and an expired Gmail lease becomes `delivery_uncertain` while an expired Calendar lease requires explicit provider reconciliation before retry. The memory ledger is limited to deterministic tests/fixture E2E; the PostgreSQL ledger is the durable runtime boundary.
