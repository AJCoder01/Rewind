...
- keyboard order from the request textarea to the primary action and a black `:focus-visible` outline verified at at least 3:1 against the paper, white, and primary-green backgrounds;
...
- selectors from `CONTROLLED_CONTENT_UI_INVENTORY.md` for the current three screens only; the one fixture E2E command starts Next with an explicit `NODE_ENV=test` fixture environment and does not inherit the caller environment or `.env.local`.
