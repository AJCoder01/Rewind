...
`tests/unit/provider-spike.test.ts` uses only `FakeCalendarPort` and `MemoryDemoEventStateStore` to prove the two-event preflight, stale `If-Match` conflict, reversible move/restore, final preflight, partial receipt summary, live-flag guard, explicit product-effect disablement, model-before-Calendar ordering, and zero Calendar calls after model failure. `tests/unit/openai-model.test.ts` verifies the versioned synthetic prompt, strict JSON Schema request, model metadata projection, absence of address-like fixture data, and one raw HTTP attempt per outer validation attempt. `tests/unit/openai-responses.test.ts` separately proves sanitized 400/401/403/404/408/409/422/429/5xx/network/output categories, retryability, timeout, and caller-owned attempt limits.

The human checkpoint is `LIVE_INTEGRATION_TESTS=1 npm run prove:provider-spikes` in a TTY. It first validates all three strict model-only outputs with the S042 validator. Only then may its Calendar phase perform one controlled UK move and restore; the deliberate US stale patch must not mutate the event. The command reports no raw provider data. Existing S035 OAuth/lookup and S038 Gmail success/replay reports are linked rather than duplicated. Product execution and reset remain disabled.
