...
Current status: master-plan tasks `S001`–`S037` are complete; `S038` is in progress at its human-only live Gmail checkpoint. S035’s deployed OAuth connection and human TTY-gated Calendar preflight passed with exactly two candidates, two persisted baselines, and two rolling provider versions. S036’s deterministic Calendar move/restore primitive packet passed without live provider writes. S037’s deterministic Gmail at-most-once packet passed without a live send. The deployed non-effecting MCP → API → PostgreSQL → dashboard proof passed without provider calls or external effects, the v1 interface packet is frozen, and G1 is closed. Gate G0 and G1 are complete; G2 is now in progress.

| Field | Value |
|---|---|
| Status | Live checklist |
| Current phase | G2 OAuth/provider/model risk retirement; next task `S038` |
...
- [~] `S038` human-gated Gmail success and replay proof: the strict `gmail-live-proof.v1` plan/action contract, TTY/live-flag/CI/production/recipient guards, fixed durable proof ledger, token-before-claim ordering, one-send/replay command, safe output, and deterministic tests are implemented. The human must run `LIVE_INTEGRATION_TESTS=1 npm run prove:gmail` and confirm exactly one inbox message before this task can close. Guide: [S038 Gmail live-proof guide](S038_GMAIL_LIVE_PROOF_GUIDE.md). Pending evidence: [S038 live-proof report](../artifacts/test-runs/2026-07-16-s038-gmail-live-proof.md).
...
|---|---|---|---|
| Live Gmail success/replay and later provider/model spikes are not yet recorded | G2 cannot close on deterministic code alone | Human runs S038 and the remaining S043 provider/model checks with the connected controlled account | Open |
