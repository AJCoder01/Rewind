...
- [~] `S043` controlled provider/model spikes: added `provider-spike.v1`, the TTY/live-flagged two-event Calendar move/restore/stale-conflict harness, the strict OpenAI model adapter and versioned synthetic prompt, deterministic tests, and the [human S043 guide](S043_PROVIDER_MODEL_SPIKE_GUIDE.md). Live Calendar/model output remains unrecorded; existing S035 OAuth/lookup and S038 Gmail/replay evidence are not duplicated. No live command was run by Codex.
...
|---|---|---|---|
| Remaining live provider/model spikes are not yet recorded | G2 cannot close on deterministic code alone | Human runs the remaining S043 checks with the connected controlled account | Open |
