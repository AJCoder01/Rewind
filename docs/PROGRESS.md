...
- [~] `S043` controlled provider/model spikes: added `provider-spike.v1`, the TTY/live-flagged two-event Calendar move/restore/stale-conflict harness, the strict OpenAI model adapter and versioned synthetic prompt, sanitized HTTP/timeout/output categories, a single two-attempt ceiling, and model-before-Calendar ordering. Human preflight passed with exactly two candidates/baselines/versions. Three combined attempts failed closed at the initial model stage (`failed_safely`, then `model_initial_invalid_output`, then `model_initial_unavailable`), revealing diagnostic loss, nested retry amplification, and repeated Calendar-before-model work; those code defects are corrected and regression-covered. Final live model/combined receipt evidence remains required. Existing S035 OAuth/lookup and S038 Gmail/replay evidence are not duplicated. No live command was run by Codex.
...
|---|---|---|---|
| Final S043 live model/combined receipt is not yet recorded | G2 cannot close on deterministic code alone; prior model failure categories were too coarse | Human runs one corrected S043 check and returns only its sanitized final JSON | Open |
