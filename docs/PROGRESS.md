...
Current status: master-plan tasks `S001`–`S034` are complete; `S035` is in progress with its safe code/test packet complete and its human-only live seed/preflight pending. The deployed non-effecting MCP → API → PostgreSQL → dashboard proof passed without provider calls or external effects, the v1 interface packet is frozen, and G1 is closed. Gate G0 and G1 are complete; G2 is now in progress.

| Field | Value |
|---|---|
| Status | Live checklist |
| Current phase | G2 OAuth/provider/model risk retirement; restart at `S035` |
...
- [~] Exactly two tagged Calendar candidates are fetched from the controlled calendar. The strict wire adapter, exact-two validator, and deterministic fake proof are complete; live discovery remains pending S035 human execution.
...
| G2 | `S031`–`S045`: OAuth, Calendar/Gmail/artifact/model primitives and live spikes | In progress | S031–S034 passed; S035 safe Calendar setup boundary is implemented; human TTY seed/preflight remains pending and no live provider call has started |
...
- [~] `S035` controlled Calendar discovery and seeding: strict `calendar-demo.v1` seed/state contracts, Google Calendar wire mapping, DST-aware two-event construction, immutable semantic-baseline and rolling-version persistence, exact-two validation, partial/provider failure auditing, and TTY/CI/production/explicit-target guards are implemented and covered by deterministic tests. Evidence: [S035 Calendar setup report](../artifacts/test-runs/2026-07-16-s035-calendar-setup.md) and [OAuth callback correction](../artifacts/test-runs/2026-07-16-s035-oauth-callback-fix.md). Requirement links: FR-04, SAFE-05, SAFE-10, NFR-04, NFR-10. A strict-query regression that blocked Google's documented `iss` callback metadata and a scope-alias normalization gap were corrected locally; Production must be redeployed before the human-only OAuth refresh, controlled database readiness, live Calendar discovery/create, and TTY preflight can run. No live provider call or external effect was run.
...
|---|---|---|---|
| OAuth token and live provider ownership are not configured | Calendar/Gmail/model risk cannot be retired | Complete S035–S043 in G2 under the human-only live-effect rules | Open |
...
Supabase is provisioned, the frozen foundation schema is applied, and S009 Vercel health/readiness and cookie checks pass. Google Cloud and OpenAI access prerequisites are configured without live product effects. S012 private environment validation and S013–S018 local and hosted verification passed, including the disposable PostgreSQL migration replay. S019–S034 pass in their documented scopes; G1 is closed and the OAuth transaction, local identity, negative-test, and provider-port boundaries are complete. The reviewed `0002_oauth_transaction` migration must be applied before the new readiness check can pass. S035 safe code is complete, but the first human-only boundary now blocks its completion: the owner must use the intended private environment and TTY-gated Calendar seed/preflight.
...
| 2026-07-16 | S035 safe Calendar setup boundary | [S035 Calendar setup report](../artifacts/test-runs/2026-07-16-s035-calendar-setup.md): `calendar-demo.v1`, strict Google wire mapping, deterministic exact-two seed/preflight tests, immutable baseline/rolling-version store, failure audit, TTY/CI/production/target refusal, and safe command output | Safe implementation and automation passed; human OAuth refresh, controlled DB verification, live Calendar discovery/create, and TTY preflight remain pending; no live provider call or external effect occurred | Codex; human owner pending |
| 2026-07-16 | S035 OAuth callback compatibility correction | [S035 OAuth callback correction](../artifacts/test-runs/2026-07-16-s035-oauth-callback-fix.md): bounded Google callback metadata acceptance, strict unknown-field rejection, focused/full tests, typecheck, lint, build, browser, security, traceability, and fake-production checks | Passed locally; Production redeploy and human OAuth retry remain pending; no live provider call or external effect occurred | Codex |
| 2026-07-16 | Provider-boundary adversarial review correction | [Sanitized review report](../artifacts/test-runs/2026-07-16-provider-boundary-adversarial-review.md): repaired the Google Calendar Event/list response projections and bound private TTY confirmation to the exact configured Calendar ID plus unique run ID; focused and full deterministic verification passed | PASS WITH NON-BLOCKING RISKS; S035 human-only controlled database/OAuth/Calendar seed/preflight remains pending; no credential, live provider call, or external effect occurred | Codex |
