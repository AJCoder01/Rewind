...
Current status: master-plan tasks `S001`–`S042` are complete and the requested broad regression audit plus the follow-up S041 CI correction passed. S035’s deployed OAuth connection and human TTY-gated Calendar preflight passed with exactly two candidates, two persisted baselines, and two rolling provider versions. S036’s deterministic Calendar move/restore primitive packet passed without live provider writes. S037’s deterministic Gmail at-most-once packet passed without a live send. S038’s human Gmail proof sent exactly one message and verified replay without redispatch. S039’s artifact boundary now binds the versioned source to independent exact bytes and persists approved content without regeneration. S040’s Responses client enforces strict structured-output transport and safe bounded failures, S041 closes all three model-only outputs over supplied ID/template universes, and S042 closes deterministic cross-field rejection/evaluation for those outputs. The deployed non-effecting MCP → API → PostgreSQL → dashboard proof passed without provider calls or external effects, the v1 interface packet is frozen, and G1 is closed. S043’s redacted provider/model spike harness and deterministic tests are prepared; its human Calendar/model evidence is still required. Gate G0 and G1 are complete; G2 remains open.

| Field | Value |
|---|---|
| Status | Live checklist |
| Current phase | G2 OAuth/provider/model risk retirement; S043 safe harness is prepared and human Calendar/model evidence is next |
| Last updated | 2026-07-16 |
| Implementation update | One sequential `S001`–`S103` plan replaces the prior person-specific workstreams; S043's live command is isolated behind TTY/live guards and no product execution/reset path is enabled. |
...
| G2 | `S031`–`S045`: OAuth, Calendar/Gmail/artifact/model primitives and live spikes | In progress | S031–S042 deterministic boundaries passed; S038 human Gmail proof passed; S043–S045 live/provider closure remains gated |
...
- [~] `S043` controlled provider/model spikes: added `provider-spike.v1`, the TTY/live-flagged two-event Calendar move/restore/stale-conflict harness, the strict OpenAI model adapter and versioned synthetic prompt, sanitized HTTP/timeout/output categories, a single two-attempt ceiling, and model-before-Calendar ordering. Human preflight passed with exactly two candidates/baselines/versions. Three attempts before the correction failed closed (`failed_safely`, `model_initial_invalid_output`, and `model_initial_unavailable`) and exposed diagnostic/retry/order defects. The corrected human attempt returned `model_initial_rate_limited` before Calendar opened, proving the code correction and isolating an OpenAI project/organization rate or quota blocker. Final live model/combined receipt evidence remains required after that external limit is resolved. Existing S035 OAuth/lookup and S038 Gmail/replay evidence are not duplicated. No live command was run by Codex.
...
|---|---|---|---|
| OpenAI returned a rate-limit/quota response before the S043 Calendar phase | G2 cannot close until all three strict model calls and the combined Calendar proof pass | Human privately resolves the configured project/organization billing or limits, then runs one corrected S043 check and returns only its sanitized final JSON | Open |
...
Supabase is provisioned, the frozen foundation schema is applied, and S009 Vercel health/readiness and cookie checks pass. Google Cloud and OpenAI access prerequisites are configured without live product effects. S012 private environment validation and S013–S018 local and hosted verification passed, including the disposable PostgreSQL migration replay. S019–S042 pass in their documented scopes; the S041 CI correction also passed hosted CI; G1 is closed and the OAuth transaction, local identity, negative-test, provider-port, controlled Calendar setup, Calendar primitive, Gmail at-most-once, human Gmail success/replay, independent artifact, strict Responses transport, closed model-only schema, and S042 semantic/evaluation boundaries are complete. S043 remains the human/provider gate before G2 can close.
