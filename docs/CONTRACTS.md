...
`prove:provider-spikes` requires the same non-production PostgreSQL/TTY/live-flag guard as the setup commands and rejects explicit product execution/reset enablement. `REWIND_S043_MODEL_RUNTIME` is mandatory for this historical admin command and independently selects either `local_ollama` or `openai_responses`; omission fails closed, and `REWIND_MODEL_RUNTIME` never substitutes for it. The local path is fixed to loopback and rejects cloud-tagged models. The confirmation phrase and target fingerprint bind the selected runtime and exact model. All three non-effecting model operations and validators complete before the Calendar phase can begin; the shared semantic retry ceiling permits at most two provider requests per operation, six total. The stale wrapper changes only the `If-Match` value sent for the deliberate conflict request; no retry or rebase is allowed. The command performs one reversible UK move and one restore, then requires the final two-event preflight to pass. S035's OAuth/lookup evidence and S038's allowlisted Gmail/replay evidence remain separate human checkpoints.

S045 adds the strict `g2-closure.v1` report and G3 admission boundary. The report contains only `passed | blocked`, the historically proved S043 model runtime/provider/evidence class, six named risk statuses, sanitized relative evidence references, a bounded blocker-code list, and `unlocked | blocked` G3 admission. The zero-cost S043 evidence is bound to `local_ollama`, provider `ollama`, evidence class `local_model`, and the verified local model name; this historical evidence field does not inspect or select the current product runtime. Current product selection is separately guarded by explicit environment validation, `verify:zero-credit-runtime`, product-factory tests, and `prove:model-local`. The verifier reads only the fixed S032–S044 evidence manifest, checks exact sanitized markers, scans those files for credential/secret/connection-URL/email patterns, and never returns matched text. Missing files, missing markers, or redaction findings make the report blocked. `assertG3Admission` rejects every blocked report; no provider, database, OAuth, Calendar, Gmail, model, or product execution call occurs in this gate.
...
`REWIND_MODEL_RUNTIME` is mandatory for PostgreSQL product planning. `openai_responses` requires both `OPENAI_API_KEY` and `OPENAI_MODEL`; `local_ollama` requires `REWIND_LOCAL_MODEL`, uses the fixed loopback endpoint, rejects `:cloud` aliases, and requires no OpenAI credentials. The historical `REWIND_S043_MODEL_RUNTIME` selector is independent and mandatory only for `prove:provider-spikes`; it never falls back to or overrides product selection, and differing explicit values are valid because they control separate commands. Store the actual returned model metadata and label local evidence `local_model`; it is real local inference, not external OpenAI evidence. `FixtureModelMetadata` remains limited to deterministic tests and the visibly non-effecting fixture contract proof. It cannot authorize or support an external effect. A fallback source remains forbidden during the recorded demo.
