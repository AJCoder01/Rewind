...
- [x] **S044 — Build honest connection/preflight UI.** Added the authenticated, read-only `connection-preflight.v1` status boundary and dashboard panel. It reports safe configuration gaps, approved connected-identity state, fixture/live-capable/blocked runtime state, database status, Calendar target/preflight failures or not-run state, configured model runtime, and explicit product execution/reset prerequisite state. It never runs a provider/model operation, claims model reachability, or claims that the product workflow passed. Evidence: [S044 connection/preflight UI report](../artifacts/test-runs/2026-07-16-s044-connection-preflight-ui.md).
...
- [x] **S055 — Execute the exact approved Gmail notification.** Added `initial-gmail-execution.v1` and the dashboard-only action-ledger executor: it requires the approved artifact and successful reversible Calendar action, validates the exact approved template/body hash/sender/allowlist, prepares locally before claiming, persists `dispatch_started_at` and redacted message-digest before handoff, and records sent, permanent, uncertain, retryable, conflict, and replay outcomes without redispatch. Evidence: [S055 Gmail-execution report](../artifacts/test-runs/2026-07-16-s055-gmail-execution.md). S056 is next.
- [x] **S056 — Build execution/timeline UX.** Added the dashboard-only `execution-timeline.v1` read boundary over the durable action ledger, with fixed initial-action labels/order, overall awaiting/in-progress/partial/attention/cancelled/failed/completed states, per-action status/attempt/timestamp/dispatch-marker fields, typed receipt rendering, redacted retry/stop explanations, strict session/MCP separation, responsive reduced-motion UI, and no-success behavior for missing/inconsistent ledger state. Evidence: [S056 execution-timeline report](../artifacts/test-runs/2026-07-16-s056-execution-timeline.md). S057 is next.
- [x] **S057 — Complete initial-workflow verification.** Added a deterministic end-to-end harness over candidate resolution, bounded reasoning, exact plan expansion, approval, durable action preparation, artifact persistence, conditional Calendar move, and allowlisted Gmail notification. It verifies exact artifact equality/independence, fixed action order, approval/digest/MCP rejection, replay without duplicate effects, safe retry/resume, duplicate-click leases, process-death reconciliation, stale ETag, allowlist drift, and honest terminal states. Evidence: [S057 initial-workflow verification report](../artifacts/test-runs/2026-07-16-s057-initial-workflow.md). S058 is next.

Pre-S058 correction packet: the product runtime now performs provider-grounded PostgreSQL intake, zero-credit local-Ollama planning without OpenAI credentials, exact dashboard approval, whole-plan preflight, synchronous artifact → Calendar → Gmail execution, pristine stale-approval supersession, request/action concurrency fencing, and fixture refusal. Canonical contracts, safety/architecture/test documentation, route/adversarial tests, and the [human S058 guide](S058_CONTROLLED_LIVE_INITIAL_FLOW_GUIDE.md) are synchronized. This correction does not complete S058 and ran no live provider effect. Evidence: [pre-S058 correction pass](../artifacts/test-runs/2026-07-17-pre-s058-correction-pass.md).

Post-correction runtime audit: PostgreSQL product planning now requires an explicit product selector, the historical S043 selector is isolated and independently mandatory, `prove:model-local` is bound to the exact product-selected model, standalone commands match Next environment precedence, stale OpenAI fields cannot redirect explicit local mode, and CI carries a deterministic zero-credit transport guard. Documentation distinguishes configured runtime from evidence/reachability and states that Vercel cannot reach a developer-laptop Ollama loopback. This audit runs no Calendar/Gmail/provider effect and does not complete S058. Evidence: [OpenAI-to-Ollama runtime audit](../artifacts/test-runs/2026-07-17-ollama-runtime-audit.md).
