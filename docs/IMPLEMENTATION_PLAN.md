...
- [x] **S013 — Add fast CI and repository security checks.** Run install, lint, typecheck, unit/contract tests, production build, dependency audit, secret scan, migration validation against ephemeral PostgreSQL, and fake-in-production checks on every change. Evidence: [sanitized S013 report](../artifacts/test-runs/2026-07-15-s013-ci-security.md); hosted ephemeral migration execution remains pending after push.
