...
| Current restart point | `S038` — prove one controlled Gmail success |
...
- [~] **S038 — Prove one controlled Gmail success.** The strict `gmail-live-proof.v1` plan/action contract, non-production/CI/TTY/live-flag and exact-recipient guards, fixed durable proof ledger, one-send/replay command, redacted output, and deterministic tests are implemented. The human must run `LIVE_INTEGRATION_TESTS=1 npm run prove:gmail`, confirm the exact target/run phrase, and verify exactly one inbox message before S038 can close. Guide: [S038 Gmail live-proof guide](S038_GMAIL_LIVE_PROOF_GUIDE.md).
