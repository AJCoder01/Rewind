...
- [~] **S043 — Run the controlled provider/model spikes.** Added the redacted `provider-spike.v1` report, TTY/live-flagged low-level Calendar move/restore/conflict harness, strict OpenAI model adapter/prompt, deterministic tests, and the [human S043 guide](S043_PROVIDER_MODEL_SPIKE_GUIDE.md). Live Calendar/model evidence remains required; S035 OAuth/lookup and S038 allowlisted Gmail/replay evidence are already recorded.
