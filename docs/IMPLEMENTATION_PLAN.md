...
- [~] **S043 — Run the controlled provider/model spikes.** Added the redacted `provider-spike.v1` report, TTY/live-flagged low-level Calendar move/restore/conflict harness, strict OpenAI model adapter/prompt, sanitized transport categories, one shared two-attempt ceiling, model-before-Calendar ordering, deterministic tests, the [human S043 guide](S043_PROVIDER_MODEL_SPIKE_GUIDE.md), [model transport correction report](../artifacts/test-runs/2026-07-16-s043-model-transport-correction.md), and [rate-limit blocker report](../artifacts/test-runs/2026-07-16-s043-openai-rate-limit-blocker.md). Human preflight passed. Three attempts before the transport correction failed closed without final evidence; the corrected attempt identified `model_initial_rate_limited` before opening Calendar. OpenAI project/organization billing or limits must be resolved before one final live model/combined run. S035 OAuth/lookup and S038 allowlisted Gmail/replay evidence are already recorded.
