...
async function main(): Promise<void> {
  ...
  try {
    ...
    const result = await preflightControlledCalendar({ calendar, state, configuration });
    process.stdout.write(`${JSON.stringify({ operation: "preflight", runId, ...result })}\n`);
  } catch (error) {
    process.stderr.write(`${JSON.stringify({ status: "failed", operation: "preflight", code: safeDemoCommandFailureCode(error) })}\n`);
    process.exitCode = 1;
  } finally {
    await pool?.end();
  }
